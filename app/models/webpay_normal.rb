...
class WebpayNormal
    ...
    def initTransaction(amount, buyOrder, sessionId, urlReturn, urlFinal)
      ...
      WebpayInit.log('init_transaction', 'buyorder:' + buyOrder , 'request', document.to_s, sessionId)
      begin
        ...
        WebpayInit.log('init_transaction',  'buyorder:' + buyOrder, 'response', response_array.to_s, sessionId)
    ...
    def getTransactionResult(token)
      ...
      #firmar la peticion
      document = sign_xml(req)
      WebpayInit.log('get_transaction_result', 'token: ' + token.to_s , 'request', document.to_s, token)
      ...
      begin
        ...
        WebpayInit.log('get_transaction_result', 'token: ' + token.to_s , 'response', response_array.to_s, token)
        return response_array
      end
      ...
      WebpayInit.log('get_transaction_result', 'token: ' + token.to_s , 'response', response_array.to_s, response_document)
    ...
    def acknowledgeTransaction(token)
      ...
      document = sign_xml(req)
      WebpayInit.log('acknowledge_transaction', 'token: ' + token.to_s , 'request', document.to_s, 'null')
      ...
      begin
        ...
        WebpayInit.log('acknowledge_transaction', 'token: ' + token.to_s , 'request', 'null', token)
      rescue Exception, RuntimeError => e
        puts "Ocurrio un error en la llamada a Webpay: "+e.message
        response_array ={
            "error_desc" => "Ocurrio un error en la llamada a Webpay: "+e.message
        }
        WebpayInit.log('acknowledge_transaction', 'token: ' + token.to_s , 'response', response_array.to_s, 'null')
        return response_array
      end
